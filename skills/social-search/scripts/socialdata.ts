...
//
// Auth: SOCIALDATA_API_KEY. Taken from the environment if set, else read from
// the manager root .envrc (so `mngr script socialdata ...` works with or without
// direnv loaded). The key is never printed.
//
// Usage:
...
function apiKey(): string {
  ...
  console.error("SOCIALDATA_API_KEY not found in env or .envrc");
  process.exit(1);
}
